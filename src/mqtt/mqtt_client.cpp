...
MqttConfig MqttSubscriber::loadMqttConfig(const std::string &filename) {
    ...
    spdlog::info("MQTT 配置文件: Host={}, Port={}, Client ID={}",
                 config_json["mqtt"]["host"].dump(),
                 config_json["mqtt"]["port"].dump(),
                 config_json["mqtt"]["client_id"].dump());
    ...
    if (!config_json.contains("mqtt") || !config_json["mqtt"].contains("client_id") ||
